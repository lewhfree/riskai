...
class User(player_class.PlayerTemplate):
    def decision(self, observation: m.Observation, phase: Stages) -> m.Response:
        print(observation)
        match phase:
            # =================================================================
            # =================================================================
            case Stages.TREATY:
                level = input("Which Treaty level (LEVEL1, LEVEL2, LEVEL3, WAR, NONE)? ")
                level = m.TreatyLevels[level]
                person = inputs.int_input("Which person to attack? ")
                response = m.Response(
                    Stages.TREATY,
                    m.Treaty(level=level, person=person)
                )
                return response
            # =================================================================
            # =================================================================
            ...
            case Stages.ATTACK_DECLARATION:
                ...
                if not shouldAttack:
                    return m.Response(Stages.ATTACK_DECLARATION, m.Attack(do_attack=False))
            ...
            case Stages.RETREAT:
                ...
                return m.Response(Stages.RETREAT, m.Retreat(retreat=shouldRetreat))
            # =================================================================
            # =================================================================
            case Stages.FORTIFY:
                fortifyq = inputs.bool_input("Should fortify stage (True/False)? ")
                if not fortifyq:
                    return m.Response(Stages.FORTIFY, m.Fortify(do_reinforce=False))
